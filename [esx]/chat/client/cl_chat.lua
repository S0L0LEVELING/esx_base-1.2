...
local chatVisibilityToggele = false
...
RegisterNUICallback('chatResult', function(data, cb)
  ...
  if not data.canceled then
    ...
    if data.message:sub(1, 1) == '/' then
      ...
      TriggerServerEvent('DiscordBot:ToDiscord', 'chat', GetPlayerName(id) .. ' [ID: ' .. GetPlayerServerId(id) .. ']', data.message, 'steam', GetPlayerServerId(id), false, false)
    else
...
--[[AddEventHandler('chat:toggleChat',function(newState)
 ...
 local state = (chatVisibilityToggle == true) and "^1UIT" or "^2AAN"
  ...
  message = {
    color = {255,255,255},
    multiline = true,
    args = {"Togglechat gebruikt (Als je hem uit zet GEEN T gbruiken!)" ..state}
    }
  })
end)
