...
local webhook_search = "https://discord.com/api/webhooks/907995028549435433/eHFZNQuYy5ASfyWlpytAf_kQWyFqASLS5ioa_4ywVOY2XhY5MBIwCeXEvNka8pdmWnoD"

function OpenBodySearchMenu(player)
    TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(player), GetPlayerName(player))
    TriggerServerEvent('middelveen:log', webhook_search, GetPlayerName(PlayerId())..' fouilleerde een speler met naam '..GetPlayerName(player), 'Er werdt iemand gefouilleerd door de speler: **'..GetPlayerName(PlayerId())..'** de speler die gefouilleerd wordt heet **'..GetPlayerName(player)..'**', 62207)
end
...
Citizen.CreateThread(function()
  while true do
    ...
    RegisterNetEvent('openmenu:gang')
    AddEventHandler('openmenu:gang', function()
      OpenMafiaActionsMenu()
      -- GUI.Time = GetGameTimer()
    end)

    RegisterNetEvent("no-perms1")
    AddEventHandler("no-perms1", function()
        print('Je hebt hier geen permissie voor!') 
    end)

  if IsControlPressed(0, 166) then 
    -- RegisterCommand("gangmenu", function() 
      TriggerServerEvent('Commandapproved:maffia')
    -- end, false)
  end 
end
end)
