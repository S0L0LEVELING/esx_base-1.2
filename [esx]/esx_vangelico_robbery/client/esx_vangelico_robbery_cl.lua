	...
	while true do
							        ...
							        ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio', function(CopsConnected)
								        if CopsConnected >= Config.RequiredCopsRob then
											ESX.TriggerServerCallback('middelveen:isrunning', function(cb)
												if not cb then
													TriggerServerEvent('esx_vangelico_robbery:rob', k, securityToken)
		...
		if holdingup then
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
					    ...
					    TriggerServerEvent('esx_vangelico_robbery:gioielli', securityToken)
						...
						if vetrineRotte == Config.MaxWindows then 
							...
							TriggerServerEvent('esx_vangelico_robbery:endrob', store, securityToken)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				TriggerServerEvent('esx_vangelico_robbery:toofar', store, securityToken)
				holdingup = false
				for i,v in pairs(vetrine) do 
					v.isOpen = false
					vetrineRotte = 0
							...
							if quantity >= Config.MaxJewelsSell then
								ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsSell then
										...
										TriggerServerEvent('lester:vendita', securityToken)
										blip = false
									else
										blip = false
