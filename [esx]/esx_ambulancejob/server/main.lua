...
RegisterServerEvent('esx_ambulancejob:doodverklaard')
AddEventHandler('esx_ambulancejob:doodverklaard', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'ambulance' then
		TriggerClientEvent('esx_ambulancejob:doodverklaard', target)
	end
end)
...
AddEventHandler('esx_ambulancejob:revive', function(playerId)
	...
	if xPlayer and xPlayer.job.name == 'ambulance' then
		...
		if xTarget then
			if deadPlayers[playerId] then
				...
				TriggerClientEvent('esx_ambulancejob:revive', playerId, source, 'notadmin')
	...
	elseif xPlayer and xPlayer.job.name == 'police' then
		local xTarget = ESX.GetPlayerFromId(playerId)

		if xTarget then
			if deadPlayers[playerId] then
				--xPlayer.showNotification(_U('revive_complete_award', xTarget.name, Config.ReviveReward2))
				--xPlayer.addMoney(Config.ReviveReward2)
				--xTarget.triggerEvent('esx_ambulancejob:revive')
...
RegisterNetEvent('SendMessageToAmbulance')
AddEventHandler('SendMessageToAmbulance', function(ambuspeler, revived)
    local xPlayer = ESX.GetPlayerFromId(ambuspeler) 

    if not revived then
        TriggerClientEvent('okokNotify:Alert', ambuspeler, "Persoon Doodverklaard", "De des betreffende persoon heeft teveel verwondingen!", 5000, 'error')
    else
        TriggerClientEvent('okokNotify:Alert', ambuspeler, "Persoon omhoog geholpen!", "U heeft €"..Config.ReviveReward..',- verdiend aan iemand omhoog helpen!', 5000, 'success')
...
if Config.EarlyRespawnFine then
	...
	AddEventHandler('esx_ambulancejob:payFine', function(key)
		local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterCommand('revive', function(source, args)
	...
	local TargetName = GetPlayerName(xTarget)

	if TargetName ~= nil then
		TriggerClientEvent('esx_ambulancejob:revive', xTarget, source, 'admin')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Command", "Deze speler is niet online!", 5000, 'error')
	end
end, true)
...
ESX.RegisterUsableItem('stretcher', function(source)
	if not playersHealing[source] then
		...
		playersHealing[source] = true
		TriggerClientEvent('esx_ambulancejob:useItem', source, 'stretcher')
