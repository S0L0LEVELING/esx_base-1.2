      ...
      if IsControlPressed(0, Keys['E']) then
        if CurrentAction == 'ambulance_duty' then
          ...
          if PlayerData.job.name == 'ambulance' then
            exports['okokNotify']:Alert("Wel thuis!", "Je bent uitgeklokt", 5000, 'error')
            Wait(1000)
          else
            exports['okokNotify']:Alert("Dienst draaien", "Je bent indienst!", 5000, 'success')
            Wait(1000)
          end
        else
          exports['okokNotify']:Alert("Oeps", "Je staat niet op de werknemerslijst", 5000, 'warning')
          Wait(1000)
        end
      end
