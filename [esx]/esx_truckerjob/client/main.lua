...
function FinishDelivery()
  if IsVehicleAttachedToTrailer(truck) and (GetVehiclePedIsIn(GetPlayerPed(-1), false) == truck) then
    ...
    isDelivered = 1
	  ...
	  TriggerEvent('esx:showNotification', "U moet de aanhanger gebruiken die voor u is voorzien.")
  end
end
...
AddEventHandler('esx_truckerjob:hasEnteredMarker', function(zone)
  if actualZone == 'menutrucker' then
    ...
    CurrentActionMsg  = 'Druk op ~INPUT_CONTEXT~ om je route te starten'
    CurrentActionData = {zone = zone}
  elseif actualZone == 'delivered' then
    CurrentAction     = 'delivered_menu'
    CurrentActionMsg  = 'Druk op ~INPUT_CONTEXT~ om je vracht te leveren'
    CurrentActionData = {zone = zone}
  elseif actualZone == 'abort' then
	  CurrentAction     = 'abort_menu'
    CurrentActionMsg  = 'Druk op ~INPUT_CONTEXT~ om je route te stoppen'
    CurrentActionData = {zone = zone}
  end
end)
