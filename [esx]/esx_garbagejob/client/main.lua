...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('esx_renewi:setconfig')
end)

RegisterNetEvent('esx:setJob')
AddEventHandler('esx:setJob', function(job)
	PlayerData.job = job
	TriggerEvent('esx_renewi:checkjob')
end)

RegisterNetEvent('esx_renewi:movetruckcount')
AddEventHandler('esx_renewi:movetruckcount', function(count)
	Config.TruckPlateNumb = count
end)

RegisterNetEvent('esx_renewi:updatejobs')
AddEventHandler('esx_renewi:updatejobs', function(newjobtable)
	CollectionJobs = newjobtable
end)


RegisterNetEvent('esx_renewi:selectnextjob')
AddEventHandler('esx_renewi:selectnextjob', function()
...
RegisterNetEvent('esx_renewi:enteredarea')
AddEventHandler('esx_renewi:enteredarea', function(zone)
...
RegisterNetEvent('esx_renewi:leftarea')
AddEventHandler('esx_renewi:leftarea', function()
...
AddEventHandler('esx_renewi:checkjob', function()
	if PlayerData.job.name ~= Config.JobName then
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			...
			sleep = 0
			TriggerEvent('esx_renewi:enteredarea', currentZone)
		end

		if not IsInArea and HasAlreadyEnteredArea then
			HasAlreadyEnteredArea = false
			sleep = 1000
			TriggerEvent('esx_renewi:leftarea', currentZone)
		end
...
function CollectBagFromBin(currentZone)
	...
	if DoesEntityExist(worktruck) and GetDistanceBetweenCoords(GetEntityCoords(worktruck), GetEntityCoords(GetPlayerPed(-1)), true) < 25.0 then
		...
		TriggerServerEvent('esx_renewi:bagremoval', currentZone.pos, currentZone.trucknumber) 
		...
		TriggerServerEvent('esx_renewi:unknownlocation', currentZone.pos)
	end
end
...
function PlaceBagInTruck(thiszone)
	...
	depositlist = nil
	truckpos = nil
	TriggerServerEvent('esx_renewi:bagdumped', binpos, truckplate)
	HasAlreadyEnteredArea = false
end
...
function SelectBinAndCrew(location)
	...
	if bin ~= 0 then
		...
		TriggerServerEvent('esx_renewi:setworkers', GetEntityCoords(bin), truckplate, truckid )
		truckpos = nil
		albetogetbags = true
		SetBlipRoute(Blips['delivery'], false)
...
function MenuVehicleSpawner()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclespawner', {
			...
			ESX.Game.SpawnVehicle(data.current.value, Config.VehicleSpawn.pos, 270.0, function(vehicle)
				...
				TriggerServerEvent('esx_renewi:movetruckcount')   
