...
RegisterServerEvent('esx_renewi:bagdumped')
AddEventHandler('esx_renewi:bagdumped', function(location, truckplate)
    ...
    if currentjobs[location] ~= nil then
        if currentjobs[location].trucknumber == truckplate then
            ...
            if currentjobs[location].bagsremaining <= 0  and currentjobs[location].bagsdropped == currentjobs[location].totalbags then
                TriggerEvent('esx_renewi:paycrew', currentjobs[location].pos)
            end
        end 
    end
end)

RegisterServerEvent('esx_renewi:setworkers')
AddEventHandler('esx_renewi:setworkers', function(location, trucknumber, truckid)
   local  _source = source
   ...
   TriggerClientEvent('esx_renewi:updatejobs', -1, currentjobs)
end)


RegisterServerEvent('esx_renewi:unknownlocation')
AddEventHandler('esx_renewi:unknownlocation', function(location)
    if currentjobs[location] ~= nil then
...
RegisterServerEvent('esx_renewi:bagremoval')
AddEventHandler('esx_renewi:bagremoval', function(location)
    if currentjobs[location] ~= nil  then
...
RegisterServerEvent('esx_renewi:movetruckcount')
AddEventHandler('esx_renewi:movetruckcount', function()
    Config.TruckPlateNumb = Config.TruckPlateNumb + 1
    if Config.TruckPlateNumb == 1000 then
        Config.TruckPlateNumb = 1
    end
    TriggerClientEvent('esx_renewi:movetruckcount', -1, Config.TruckPlateNumb)
end)

RegisterServerEvent('esx_renewi:setconfig')
AddEventHandler('esx_renewi:setconfig', function()
    TriggerClientEvent('esx_renewi:movetruckcount', -1, Config.TruckPlateNumb)
    TriggerClientEvent('esx_renewi:updatejobs', -1, currentjobs)
end)
    ...
    _source = source
     for i, v in pairs(currentjobs) do
        if v.jobboss == _source then
            TriggerEvent('esx_renewi:paycrew', v.pos)
            removenumber = i
        end
...
AddEventHandler('esx_renewi:paycrew', function(number)
    ...
    for i, v in pairs(currentcrew) do
        ...
        if xPlayer ~= nil then
            ...
            TriggerClientEvent('esx:showNotification',i, 'Je hebt '..tostring(amount)..' ontvangen van deze stop!')
        end
    end
    local currentboss = currentjobs[number].jobboss
    currentjobs[number] = nil
    TriggerClientEvent('esx_renewi:updatejobs', -1, currentjobs)
    TriggerClientEvent('esx_renewi:selectnextjob', currentboss )
end)
