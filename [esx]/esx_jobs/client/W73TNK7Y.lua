...
AddEventHandler('esx_werk:action', function(job, zone, zoneIndex)
	menuIsShowed = true
	...
	elseif zone.Type == 'work' then
		...
		if IsPedOnFoot(playerPed) then
			TriggerServerEvent('esx_werk:startWork', zoneIndex)
		else
					...
					if IsPedInAnyVehicle(playerPed, false) then
						...
						if playerPed == driverPed then
							if myPlate[plate] then
								...
								TriggerServerEvent('esx_werk:caution', 'give_back', giveBack, 0, 0)
...
AddEventHandler('esx_werk:hasExitedMarker', function(zone)
	TriggerServerEvent('esx_werk:stopWork')
	hintToDisplay = nil
	menuIsShowed = false
	hintIsShowed = false
	isInMarker = false
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = nil
	hintIsShowed = false
	TriggerServerEvent('esx_werk:caution', 'take', vehicleCaution, spawnPoint, vehicle)
end

RegisterNetEvent('esx_werk:spawnJobVehicle')
AddEventHandler('esx_werk:spawnJobVehicle', function(spawnPoint, vehicle)
	local playerPed = PlayerPedId()
		...
		if PlayerData.job then
			...
			if IsControlJustReleased(0, 38) and not menuIsShowed and isInMarker then
				if onDuty or currentZone.Type == 'cloakroom' or PlayerData.job.name == 'reporter' then
					TriggerEvent('esx_werk:action', PlayerData.job.name, currentZone, currentZoneIndex)
				end
			end
			...
			if not isInMarker and hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_werk:hasExitedMarker', currentZone)
			end
		end
