	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				...
				exports['mythic_notify']:DoHudText('error', 'Je bent gestopt met vissen!')
			end
			if fishing then
				HideHudComponentThisFrame(19) -- HUD Weapon Wheel
				HideHudComponentThisFrame(20) -- HUD Weapon Wheel Stats
				...
				if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 or IsPedInAnyVehicle(GetPlayerPed(-1)) then
					...
					fishing = false
					--ESX.ShowNotification("~r~Stopped fishing")
					exports['mythic_notify']:DoHudText('error', 'Gestopt met vissen!')
				end
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					exports['mythic_notify']:DoHudText('error', 'De vis is vrij gekomen!')
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~De vis bijt op het aas \n ~h~Druk op " .. correct .. " om de vis te vangen.")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		...
		exports['mythic_notify']:DoHudText('error', 'Je kan niet vissen vanuit een voertuig!')
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			--ESX.ShowNotification("~g~Fishing started")
			exports['mythic_notify']:DoHudText('error', 'Je bent gestart met vissen!')
			TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
		else
			--ESX.ShowNotification("~y~Je moet veder van de kust zijn.")
			exports['mythic_notify']:DoHudText('error', 'Je moet veder van de kust zijn!')
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Druk op [E] om een boot te huren.')
