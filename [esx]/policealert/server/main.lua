...
AddEventHandler('esx_outlawalert:carJackInProgress', function(targetCoords, streetName, vehicleLabel, playerGender)
    mytype = 'police'
    data = {["code"] = '10-16', ["name"] = 'Er word een auto gestolen!'..vehicleLabel..'.', ["loc"] = streetName}
    length = 3500
...
AddEventHandler('esx_outlawalert:gunshotInProgress', function(targetCoords, streetName, playerGender)
	mytype = 'police'
    data = {["code"] = '10-11', ["name"] = 'Schoten gelost!', ["loc"] = streetName}
    length = 3500
    TriggerClientEvent('esx_outlawalert:outlawNotify', -1, mytype, data, length)
    TriggerClientEvent('esx_outlawalert:gunshotInProgress', -1, targetCoords)
end, false)

RegisterServerEvent('esx_outlawalert:askForBackup')
AddEventHandler('esx_outlawalert:askForBackup', function(targetCoords, streetName)
	mytype = 'police'
    data = {["code"] = '10-78', ["name"] = ' Agent vraagt backup', ["loc"] = streetName}
    length = 12000
    TriggerClientEvent('esx_outlawalert:outlawNotify', -1, mytype, data, length)
    TriggerClientEvent('esx_outlawalert:policeBackup', -1, targetCoords)
end, false)

RegisterServerEvent('esx_outlawalert:AgentNeer')
AddEventHandler('esx_outlawalert:AgentNeer', function(targetCoords, streetName)
	mytype = 'police'
    data = {["code"] = '10-78', ["name"] = ' Agent neer', ["loc"] = streetName}
    length = 20000
    TriggerClientEvent('esx_outlawalert:outlawNotify', -1, mytype, data, length)
    TriggerClientEvent('esx_outlawalert:policeBackup', -1, targetCoords)
end, false)
