...
RegisterNetEvent('esx_outlawalert:policeBackup')
AddEventHandler('esx_outlawalert:policeBackup', function(targetCoords)
	if isPlayerWhitelisted and Config.GunshotAlert then
		local alpha = 250
		local gunshotBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, 250.0)
		local officerBlip = AddBlipForCoord(targetCoords.x, targetCoords.y, targetCoords.z)
		...
		AddTextEntry('officerBlip', 'Agent vraagt Backup')

		BeginTextCommandSetBlipName('officerBlip')
		EndTextCommandSetBlipName(officerBlip)
		SetBlipFlashes(officerBlip, true)
