...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_customcardealer:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_customcardealer:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)

	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'customcardealer' then
...
RegisterNetEvent('esx_customcardealer:sendCategories')
AddEventHandler('esx_customcardealer:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_customcardealer:sendVehicles')
AddEventHandler('esx_customcardealer:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_customcardealer:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('esx_customcardealer:returnProvider', data.current.value)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[esx_customcardealer] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[esx_customcardealer] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_customcardealer:buyCarDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
					...
					ESX.TriggerServerCallback('esx_customcardealer:buyVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('esx_rekening:sendBill', GetPlayerServerId(closestPlayer), 'society_customcardealer', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('esx_customcardealer:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								...
								SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
								TriggerServerEvent('esx_customcardealer:rentVehicle', model, newPlate, amount, GetPlayerServerId(closestPlayer))
								currentDisplayVehicle = nil
							else
								ESX.ShowNotification(_U('no_players'))
							end
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('esx_customcardealer:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'customcardealer', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			ESX.TriggerServerCallback('esx_customcardealer:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_customcardealer:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_customcardealer:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_customcardealer:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_customcardealer:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('esx_customcardealer:hasEnteredMarker', function(zone)
	if zone == 'ShopEntering' then

		if Config.EnablePlayerManagement then
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'customcardealer' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'customcardealer' and ESX.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_customcardealer:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_customcardealer:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_customcardealer:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_customcardealer:giveBackVehicle', function(isRentedVehicle)
