...
RegisterServerEvent('esx_rekening:sendBill')
AddEventHandler('esx_rekening:sendBill', function(playerId, sharedAccountName, label, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xTarget = ESX.GetPlayerFromId(playerId)
	amount = ESX.Math.Round(amount)

	TriggerEvent('esx:factuur', xPlayer.name, xTarget.name, amount)
	TriggerEvent('logs:facturenverstuurdlog', xPlayer.name, xTarget.name, amount, label, sharedAccountName)
...
ESX.RegisterServerCallback('esx_rekening:getBills', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_rekening:getTargetBills', function(source, cb, target)
...
ESX.RegisterServerCallback('esx_rekening:payBill', function(source, cb, billId)
	local xPlayer = ESX.GetPlayerFromId(source)
					...
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								TriggerEvent('logs:facturenbetaaldlog', xPlayer.source, target, amount, label)
