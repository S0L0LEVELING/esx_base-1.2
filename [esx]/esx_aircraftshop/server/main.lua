...
ESX.RegisterServerCallback('esx_aircraftshop:buyVehicle', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, garage_name, type) VALUES (@owner, @plate, @vehicle, @garage_name, @type)', {
			['@owner']   = xPlayer.identifier,
			['@plate']   = plate,
			['@vehicle'] = json.encode({model = GetHashKey(model), plate = plate}),
			['@garage_name'] = 'Vliegveld',
			['@type']    = 'airplane'
	...
	sendToDiscord("https://discord.com/api/webhooks/904365369916026931/cUtFBuh_egyXz0Jyq2J25UOHkHKpVSe9oDhMewWRfdd95gkR6O5DQ1VEjszVyso0gzJJ", xPlayer.name..' Kocht een luchtvaartuig', 'Speler **'..GetPlayerName(source)..'** **('..GetPlayerIdentifier(source)..')** heeft luchtvaartuig gekocht met nummerplaat **'..plate..'**',16711680)
end)
