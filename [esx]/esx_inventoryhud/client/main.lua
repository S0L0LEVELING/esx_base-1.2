...
function openInventory()
    ...
    if canOpenInventory then -- checks if inventory is being searched (can be opened)
        ...
        exports['okokNotify']:Alert("Inventaris", "Je kunt je inventaris niet openen, staff of Politie is ermee bezig.", 5000, 'warning')
    end
end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        TriggerServerEvent('toDiscord', '```diff\n- Item Used\n```\n```css\n[By]: '..name.. '\n[Item]: '..data.item.name..'```\n', 'https://discord.com/api/webhooks/899047474042388551/XZrnFhpmLC2R3htji7J4PMy5KyoQ-9ZFgmJopd34xsca86Rp0gpgKR6TEZ8lJJAFbmXP')
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
			if data.item.type == "item_money" then
                ...
                TriggerServerEvent('toDiscord', '```diff\n- Item Dropped\n```\n```css\n[By]: '..name.. '\n[Dropped Item]: '..data.item.name..'```\n', 'https://discord.com/api/webhooks/899047629923684373/307AC6JhSoPdJP0g_b8DXMRiCK76OjFfrQ7Kt_D0-57nHymL-AnBh3d8vMQSeLPH4wYh')
			else
