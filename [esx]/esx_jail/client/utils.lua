--[[RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		exports['b1g_notify']:Notify('false', 'Je bent geen Politie Agent!')
	end
end)]]
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" then
					exports['okokNotify']:Alert("Politie:", "Je hebt geen sleutel om hier te komen!", 5000, 'info')
					--exports['b1g_notify']:Notify('false', 'Je hebt geen sleutel om hier te komen!')
					return
				end
			end
