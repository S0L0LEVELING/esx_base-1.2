...
function Start(ped)
	...
	while checking do
		...
		if distance < 2.0 then
			DrawText3D(x,y,z, 'Druk op [~g~E~s~] om te onderzoeken', 0.4)
...
function Notification(x,y,z)
	...
	while (timestamp + 4500) > GetGameTimer() do
		Citizen.Wait(0)
		DrawText3D(x, y, z, 'De wond lijkt hier te zitten', 0.4)
		checking = false
	end
end
...
function OpenDeathMenu(player)
	...
	table.insert(elements, {label = 'Oorzaak onderzoeken', value = 'deathcause'})
	table.insert(elements, {label = 'Wond onderzoeken', value = 'damage'})
	...
	function(data, menu)
		...
		if ac == 'damage' then
			...
			if success then
				...
				exports['b1g_notify']:Notify('dispatch', 'Oorzaak kon niet worden ontdekt')
			end
		end
		...
		if ac == 'deathcause' then
			...
			if checkArray(Melee, d) then
				exports['b1g_notify']:Notify('dispatch', _U('hardmeele'))
			elseif checkArray(Bullet, d) then
				exports['b1g_notify']:Notify('dispatch', _U('bullet'))
			elseif checkArray(Knife, d) then
				exports['b1g_notify']:Notify('dispatch', _U('knifes'))
			elseif checkArray(Animal, d) then
				exports['b1g_notify']:Notify('dispatch', _U('bitten'))
			elseif checkArray(FallDamage, d) then
				exports['b1g_notify']:Notify('dispatch', _U('brokenlegs'))
			elseif checkArray(Explosion, d) then
				exports['b1g_notify']:Notify('dispatch', _U('explosive'))
			elseif checkArray(Gas, d) then
				exports['b1g_notify']:Notify('dispatch', _U('gas'))
			elseif checkArray(Burn, d) then
				exports['b1g_notify']:Notify('dispatch', _U('fire'))
			elseif checkArray(Drown, d) then
				exports['b1g_notify']:Notify('dispatch', _U('drown'))
			elseif checkArray(Car, d) then
				exports['b1g_notify']:Notify('dispatch', _U('caraccident'))
			else
				exports['b1g_notify']:Notify('dispatch', _U('unknown'))
			end
		end
