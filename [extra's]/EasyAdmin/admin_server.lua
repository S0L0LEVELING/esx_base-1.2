...
RegisterCommand("ea_generateSupportFile", function(source, args, rawCommand)
	if DoesPlayerHavePermission(source,"easyadmin.manageserver") then
		...
		supportData.config = {
			...
			ea_MenuButton = GetConvar("ea_MenuButton", 57),
...
Citizen.CreateThread(function()
	...
	AddEventHandler('EasyAdmin:amiadmin', function()
		...
		local key = GetConvar("ea_MenuButton", 57)
		if RedM then
	...
	RegisterCommand("calladmin", function(source, args, rawCommand)
		if GetConvar("ea_enableCallAdminCommand", "false") == "true" then
			...
			if cooldowns[source] and cooldowns[source] > (time - cooldowntime) then
				TriggerClientEvent('b1g_notify:client:Notify', i, { type = 'adm', text = 'Je moet even wachten voor dat je een nieuw bericht stuurt!' })
				return
			end
			...
			for i,_ in pairs(OnlineAdmins) do 
				TriggerClientEvent('chat:addMessage', i, { 
					template = '<div style="padding: 0.3vw; margin: 0.1vw; background-color: rgba(252, 3, 3, 0.6); border-radius: 3px;"><i class="fas fa-flag"></i> {0} </div>',
					args = { "^3!!Calladmin!!^7\n"..string.format(string.gsub(GetLocalisedText("playercalledforadmin"), "```", ""), getName(source), source, reason) }, color = { 255, 255, 255 } 
				})
				TriggerClientEvent('b1g_notify:client:Notify', i, { type = 'adm', text = 'Er is een CallAdmin binnen gekomen!!' })
			end
			...
			TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'true', text = GetLocalisedText("admincalled") })
			time = os.time()
	...
	RegisterCommand("report", function(source, args, rawCommand)
		if GetConvar("ea_enableReportCommand", "true") == "false" then
			local time = os.time()
			...
			for i,_ in pairs(OnlineAdmins) do 
				...
				TriggerClientEvent('b1g_notify:client:Notify', i, { type = 'adm', text = 'Er is een Report binnen gekomen!!' })
			end
			local preferredWebhook = (reportNotification ~= "false") and reportNotification or moderationNotification
			SendWebhookMessage(preferredWebhook,string.format(GetLocalisedText("playercalledforadmin"), getName(source), source, reason), "Report")
			TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'true', text = 'Report Verzonden'  })
			time = os.time()
			cooldowns[source] = time
		end
	end, false)
	...
	RegisterServerEvent("EasyAdmin:screen")
	AddEventHandler('EasyAdmin:screen', function()
		local src = source
		...
		exports["discord-screenshot"]:requestCustomClientScreenshotUploadToDiscord(src,
			"https://discord.com/api/webhooks/945976244955144192/oHWIAgWY2XZOhPZh5H_zLQQtwJZ0DG8RMd0qokeDwd5Jp5w8pkjYZF1Y6_wL-1x0nuqu",
			{
				encoding = "png",
				quality = 1
			},
			{
				username = "MiddelveenRP",
				avatar_url = "https://cdn.discordapp.com/attachments/945356975355609128/945357310656651294/Middelveen_sun5.png",
				embeds = {
					{
						color = 16771584,
						author = {
							name = "[" .. src .. "] "  .. GetPlayerName(src).. " ",
						},
...
WarnedPlayers = {}
cooldowns = {}
local slashr_wh = "https://discord.com/api/webhooks/945976844186959922/AxLxAWxiXLJZMn4SDK2b8KGp0X4LSXE1rJsK9Kct2sxL19cO0eW65OIO17mpbf1O0PbX"
-- DO NOT TOUCH THESE
-- DO NOT TOUCH THESE
-- DO NOT TOUCH THESE
-- DO NOT TOUCH THESE
	...
	if cm[1] == "/r" then
		...
		if tablelength(cm) > 1 then
			...
			if IsPlayerAceAllowed(source, "no.kick") then
				...
				TriggerEvent('textsen', source, names2, names3, tPID, textmsg)
				TriggerEvent("middelveen:log", slashr_wh, "Iemand reageerde op een report of persoon", "Afzender: "..GetPlayerName(source).."\n Bericht: "..textmsg.."\n Ontvanger ID: "..tPID.."\n Ontvanger: "..GetPlayerName(tPID), 16711680)
			end
		end
	end		
end, true)
...
RegisterServerEvent("textsen")
AddEventHandler('textsen', function(source, names2, names3, tPID, textmsg )
	for i,_ in pairs(OnlineAdmins) do 
	TriggerClientEvent('chat:addMessage', i,{
...
RegisterCommand('sc', function(source, args, rawCommand)
	...
	for i,_ in pairs(OnlineAdmins) do 
	TriggerClientEvent('chat:addMessage', i, { 
		template = '<div style="padding: 0.3vw; margin: 0.1vw; background-color: rgba(255, 55, 0, 0.8); border-radius: 3px;"><i class="far fa-comment-dots"></i> {0}:<br> {1}</div>', --{0}:<br> {1}</div>
		args = { (name), '^*'..args }, color = { 255, 255, 255 } -- 128, 128, 128
	})
  end
end, true)
...
RegisterCommand('test', function(source, args, rawCommand)
	...
	local name = GetPlayerName(source)
	for i,_ in pairs(OnlineAdmins) do 
	TriggerClientEvent('rocket-notify:grootnotify', i, name, args, 'fas fa-wrench', '5000', '#db910f', true)
	end
end, true)

--TP een ID naar UWV
RegisterCommand('uwv', function(source, args, rawCommand)
  ...
  if (GetPlayerName(tonumber(target))) == nil or GetPlayerPing(target) == 0 then
	if source ~= 0 then
		TriggerClientEvent('EasyAdmin:error', source, 'Dit ID is niet online!')
		end
		return
	end

	TriggerClientEvent('EasyAdmin:TeleportUWV', target)
end, true)

--TP allespelers naar UWV
RegisterCommand('uwvall', function(source, args, rawCommand)
	TriggerClientEvent('EasyAdmin:TeleportUWV', -1)
end, true)

--TP een ID naar BP
RegisterCommand('bp', function(source, args, rawCommand)
	local target = tonumber(args[1])
  ...
  if (GetPlayerName(tonumber(target))) == nil or GetPlayerPing(target) == 0 then
	...
	TriggerClientEvent('EasyAdmin:TeleportBP', target)
end, true)