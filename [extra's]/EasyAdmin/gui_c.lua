		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				if result ~= GetCurrentResourceName() and result ~= "NativeUI" then
					...
					TriggerEvent("chat:addMessage", { args = { "MiddelveenRP", GetLocalisedText("badidea") } })
				end
			end
		end
	...
	if permissions["unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if foundBan then
				...
				mainMenu = NativeUI.CreateMenu("", "~b~Ban Infos", menuOrientation, 0)
				_menuPool:Add(mainMenu)
	...
	while true do
		...
		if drawInfo then
			...
			if IsPedInAnyVehicle(targetPed, true) then
				table.insert(text,GetLocalisedText("vehicletrue"))
			else
				table.insert(text,GetLocalisedText("vehiclefalse"))
			end
			if IsPedCuffed(targetPed, true) then
				table.insert(text,GetLocalisedText("boeientrue"))
			else
				table.insert(text,GetLocalisedText("boeienfalse"))
			end
