            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Steel het contante geld!", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Steel het contante geld!", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Steel het contante geld!", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "kmar" then
        exports["mythic_notify"]:SendAlert("inform", "Alarmen van de ING bank gaan af!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
        if not DoesBlipExist(blip) then
            blip = AddBlipForCoord(UTK.Banks[name].doors.startloc.x, UTK.Banks[name].doors.startloc.y, UTK.Banks[name].doors.startloc.z)
            SetBlipSprite(blip, 161)
            SetBlipScale(blip, 2.0)
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Ontgrendel de deur", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Vergrendel de deur", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["mythic_notify"]:SendAlert("error", "Kluis deur gaat dicht binnen 30 seconden!")
    Citizen.Wait(30000)
    exports["mythic_notify"]:SendAlert("error", "Kluis aan het sluiten!")
    TriggerServerEvent("utk_fh:toggleVault", name, true)
    TriggerEvent("utk_fh:cleanUp", data, name)
    TriggerServerEvent("middelveen:endrobbery")
end)

AddEventHandler("utk_fh:startheist", function(data, name)
    ESX.TriggerServerCallback('middelveen:isrunning', function(cb)
        if not cb then
            ...
            exports['progressBars']:startUI(2000, "Gebruikt Malicious Card")
            TriggerServerEvent("middelveen:startrobbery")
            Citizen.Wait(1500)
            ...
            exports["mythic_notify"]:SendAlert("success", "Hack gedaan!")
            PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
            TriggerServerEvent("utk_fh:toggleVault", name, false)
            startdstcheck = true
            currentname = name
            exports["mythic_notify"]:SendAlert("error", "Je hebt 2 minuten tot het alarm activeert.")
            SpawnTrolleys(data, name)
        else 
            ESX.ShowNotification("Er is al een overval bezig! Je kunt er nu geen starten!")
        end 
    end)
end)
...
function SecondDoor(data, key)
    ...
    exports["mythic_notify"]:SendAlert("success", "Access compleet!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:SendAlert("error", "Je hebt geen Secure ID Card.")
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Start bank overval", 0.40)
