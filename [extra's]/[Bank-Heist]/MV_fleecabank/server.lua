...
AddEventHandler("utk_fh:startcheck", function(bank)
    ...
    local item = xPlayer.getInventoryItem("id_card")["count"]
        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    xPlayer.removeInventoryItem("id_card", 1)
                    TriggerEvent("middelveen:log", UTK.startwebhook, "Iemand heeft de __FLEECA BANK__ gestart!", "Naam Speler: "..GetPlayerName(source).."\nSpeler ID: "..source, 62207)
